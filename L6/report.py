...
def _l4_section(sha: str) -> str:
    """AI-Assisted Code Analysis — L4's agentic verdicts, report-only.

    Per `decisions/plan_l4_agentic_verdicts.md`: these scores rank classes for
    a human reader's attention. They are not evidence and do not feed L5 —
    that edge is deferred behind the benign-corpus gate (T24). Every kept
    claim shown here already survived mechanical re-derivation
  ...
  {f"<p style='margin:.6rem 0 .2rem'><strong>Dropped claims</strong></p><ul style='margin:.2rem 0'>{dropped_html}</ul>" if dropped else ''}
    ...
    return (
        "<p class='muted' style='margin:0 0 .8rem'>Model-generated, report-only — "
        "scores rank classes for review and do not contribute to the score above "
        "or feed any gate. Every claim shown already survived mechanical "
        "re-derivation against this sample's own bytes.</p>"
        + "".join(cards)
    )
...
analysis section is the one exception — it is model-generated, every checkable
claim in it was mechanically re-derived and discarded if wrong, and none of it
contributes to the score or any gate above.
