...
class SentinelNetworkAddon:
    """Captures all traffic flowing through the proxy and emits JSON events."""
    ...
    def __init__(self):
        ...
        self.sensitive_patterns = [
            (r"(?i)(otp|mpin|pin|password|passw|secret|token|auth|session|cookie)",
             "credential_keyword"),
            (r"(?i)(phone|mobile|imei|imsi|iccid|mac|serial)",
             "device_identifier"),
            (r"(?i)(sms|message|sms_|read_sms|send_sms|intercept)",
             "sms_data"),
            (r"(?i)(contact|phonebook|call_log|calllog)",
             "contact_data"),
            (r"(?i)(location|latitude|longitude|gps|coarse_location)",
             "location_data"),
            (r"(?i)(bank|account|ifsc|upi|vpa|balance|transaction)",
             "financial_data"),
            (r"(?i)(keylog|clipboard|accessibility|overlay)",
             "ui_theft"),
        ]
        self.c2_patterns = [
            ...
            (r"(?i)(bit\.ly|tinyurl\.com|t\.co)", "URL shortener (redirect chain)"),
            (r"(?i)(pastebin\.com/api/)", "Pastebin API"),
        ]
    ...
    def response(self, flow: http.HTTPFlow):
        ...
        # Clean old entries (keep last 5 min)
        cutoff = now - 300
        self.beacon_tracker[beacon_key] = [
            t for t in self.beacon_tracker[beacon_key] if t > cutoff
        ]
        ...
        if len(times) >= 3:
            ...
            # Low variance + regular interval = likely beacon
        ...
        flow_record = {
            ...
            "response_headers": {k: v for k, v in resp_headers.items()
                                 if k.lower() in ("content-type", "set-cookie", "server",
                                                   "x-powered-by")},
